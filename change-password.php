...
check_login();
echo $ai = $_SESSION['login'];
// code for change password
if (isset($_POST['changepwd'])) {
	$op = $_POST['oldpassword'];
	$np = $_POST['newpassword'];
	$udate = date('d-m-Y h:i:s', time());;
	$sql = "SELECT password FROM registration where password=?";
	...
	if ($row_cnt > 0) {
		$con = "update registration set password=?  where id=?";
		$chngpwd1 = $mysqli->prepare($con);
		$chngpwd1->bind_param('ss', $np, $ai);
		$chngpwd1->execute();
									...
									<div class="panel-body">
										<form method="post" class="form-horizontal" name="changepwd" id="change-pwd" onSubmit="return valid();">
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="password" class="form-control" name="newpassword" id="newpassword" value="" required="required" onblur="checkPassword(this)">
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-4 control-label">Confirm Password</label>
												<div class="col-sm-8">
													<input type="password" class="form-control" value="" required="required" id="cpassword" name="cpassword" onblur="valid(this)">
												</div>
											</div>


<br>
													<p style="color: red">Note:In case you forget your password, it's important to check whether your email address is properly set up in your my profile tab.</p>
